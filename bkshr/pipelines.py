...
class BkshrPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('bkshr.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bkshr`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bkshr WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bkshr (date, title, link, content)"
